...
def main():
    ...
    own_pcap_1 = "own-pcap/bm-2.pcap"
    f = open(own_pcap_1, "rb")
    ...
    pgroup = pg.Pgroup(colorspace=colorspace,
                       sampling=sampling,
                       depth=depth)
    image_buf = np.zeros((video_height, video_width, 3), dtype=np.uint8)
    packet_number = 0
    for _, buf in pcap:
        packet_number += 1
        ...
        current_offsets = {}
        for i in range(0, len(st_payload), pgroup.size):
            if i + pgroup.size > len(st_payload):
                break
            if srd_offset not in current_offsets:
                current_offsets[srd_offset] = srd_offset
            #Pixel components
            pixels = pgroup.get_pixels(st_payload[i:i+pgroup.size])
            for p in pixels:
                image_buf[srd_row, current_offsets[srd_offset]] = p
